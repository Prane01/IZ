						...
						for(ExpandGRMRelationsData2 relData : relOut.relationshipData) {
							for(ExpandGRMRelationship relShip : relData.relationshipObjects) {
								if(relShip.relation instanceof ImanRelation) {
									...
									if(relShip.otherSideObject.getUid() == specificUid) {
										System.out.println("** Found UID ***");
										Connect.dmService.getProperties(new ModelObject[] {relShip.relation}, new String[] {"typed_ref"});
										
										ModelObject typed_refObj = (ModelObject) relShip.relation.getPropertyObject("");
										Connect.dmService.getProperties(new ModelObject[] {typed_refObj}, new String[] {"object_name"});
										String typed_refObjName = typed_refObj.getPropertyDisplayableValue("object_name");
										System.out.println("typed_refObjName object_name : " + typed_refObjName);
										
										Map<String,VecStruct> hmInputMapper = new HashMap<String, VecStruct>();
										VecStruct vecStr = new VecStruct();
										
										String[] entries = {"Dataset Name"};
										String[] values = {"Test_SOA_Dataset"};
										
										QueryManagement qryMgmt = new QueryManagement();
										ModelObject[] datasets = qryMgmt.getObjects("Dataset Name", entries, values)
										
										vecStr.stringVec = new String[] { datasets[0].getUid()};
										
										// properties using map
										hmInputMapper.put("typed_ref", vecStr);
										Connect.dmService.setProperties(new ModelObject[] { typed_refObj }, hmInputMapper);
