...
public class AppXSession
{
    ...
    public AppXSession(String host)
    {
        ...
        //connection.setOption(arg0, arg1);
     ...
     *
     */
    public User login(String sUserId, String sPassword, FileWriter writer) throws Exception
    {
           ...
           // String[] credentials = credentialManager.promptForCredentials();
            while (true)
            {
                try
                {
                	...
                	//LoginResponse out = sessionService.login(sUserId, sPassword,
                   //        "", "","", "SoaAppX");
                	
                	//LoginResponse out = sessionService.login(sUserId, sPassword,
                    //        "", "","", "Soax1"+new Random().nextInt());
                	
                	LoginResponse out = sessionService.login(sUserId, sPassword, "", "","", "Soax1"+new Random().nextInt());
                	
                	
                	if(out.user!=null)
                	{
                		StateNameValue[] stateValues = new StateNameValue[1];
                		stateValues[0] = new StateNameValue();
                		stateValues[0].name="bypassFlag";
                		stateValues[0].value=Property.toBooleanString(true);
                		sessionService.setUserSessionState(stateValues);
                	}
                    return out.user;
                }
        ...
        catch (/*CanceledOperation*/Exception e) {
        	 System.out.println("\nERROR:LOGIN=Please contact Teamcenter administrator!!");
         	writer.write("\nERROR:LOGIN="+e.getMessage());
        }
