...
public class CreateItems {
	public static void itemsCreate(ArrayList<String> createObject, FileWriter writer) {
		...
		try {
			...
			createIn[0].clientId = "random";

			// CreateInput for Property inputs
			CreateInput cInput = new CreateInput();
			...
			for (String id : createObject) {
				...
				cInput.stringProps = hmInputMapper;
				cInput.boName = "Item";
				createIn[0].data = cInput;
				// Call the SOA service to create the Item
				CreateResponse itemsResponse = TestConnect.dmService.createObjects(createIn);
