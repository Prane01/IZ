...
int ITK_user_main(int argc, char* argv[])
{
	...
	if ((tc_strcmp(uname, "") != 0) && (tc_strcmp(pass, "") != 0) && (tc_strcmp(grp, "") != 0)) {  // Validates all values are filled
		if (ITK_init_module(uname, pass, grp) == ITK_ok) {
			...
			ifail = ITEM_find_rev("000344", "A", &tRevisionL1);
			checkNullTag(tRevisionC2);
			...
			for (int i = 0; i < childCountLvl2; i++)
			{
				...
				if(strcmp(bl_item_id, "000338") == 0)
				{
					BOM_line_add_substitute(childrenLvl2[i], NULLTAG, tRevisionL1, NULLTAG, &newLine);
					//BOM_line_prefer_substitute(newLine, false);
					BOM_line_show_substitutes(newLine);
