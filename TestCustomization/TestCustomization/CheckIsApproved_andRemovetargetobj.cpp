...
void getDeliverableFilesFromDeliverables(tag_t tQC_approANDrejObj, tag_t t_release_status_QCOneReject);
...
//Function for Creating Deliverable response object
int CreateResponseObj(tag_t document)
{
	...
	try
	{
		//Creating deliverable response object
		TC_write_syslog("Creating Deliverable Response objects.....\n");

		tag_t tDelResponseObj = NULLTAG;
		status = TCTYPE_ask_type("H4_Deliverable_Response_Obj",&tDelResponseObj);

		if(tDelResponseObj!=NULLTAG)
		{
			//Constructing create input
			tag_t tCreateDROInput = NULLTAG;
			status = TCTYPE_construct_create_input(tDelResponseObj,&tCreateDROInput);

			if(tCreateDROInput!=NULLTAG)
			{
				//Collecting property values from Document Revsision
				scoped_smptr<char> sObjectNameDOC;
				status = AOM_ask_value_string(document,"object_name",&sObjectNameDOC);

				scoped_smptr<char> sReviewStatusDOC;
				status = AOM_ask_value_string(document,"h4_review_status",&sReviewStatusDOC);

				scoped_smptr<char> sCommentDOC;
				status = AOM_ask_value_string(document,"h4_approv_reject_comment", &sCommentDOC);

				scoped_smptr<char> sQCReviewState;
				status = AOM_ask_value_string(document,"h4_qc_review_state",&sQCReviewState);

				//Setting property values in Response object create input
				status = AOM_set_value_string(tCreateDROInput, "object_name", sObjectNameDOC.getString());
				status = AOM_set_value_string(tCreateDROInput, "h4_review_status", sReviewStatusDOC.getString());
				status = AOM_set_value_string(tCreateDROInput, "h4_review_comments", sCommentDOC.getString());
				status = AOM_set_value_string(tCreateDROInput, "h4_qc_review_state", sQCReviewState.getString());

				//Createing Response object
				tag_t tNewDROobject = NULLTAG;
				status = TCTYPE_create_object(tCreateDROInput,&tNewDROobject);
				status = AOM_save_without_extensions(tNewDROobject);

				if(tNewDROobject!=NULLTAG)
				{
					TC_write_syslog("Response Object creation successfull.....\n");

					//Attaching Document Revision to new Response object
					TC_write_syslog("Attaching Document Revision to new Response object.....\n");
					status = AOM_refresh(tNewDROobject,TRUE);
					status = AOM_set_value_tag(tNewDROobject,"h4_received_object",document);
					status = AOM_save_without_extensions(tNewDROobject);
					status = AOM_refresh(tNewDROobject,FALSE);
				}
			}
...
//Release Deliverable Files of Deliverable Document Revision and Design Revision
void getDeliverableFilesFromDeliverables(tag_t tQC_approANDrejObj,tag_t t_release_status_QCOneReject){
	TC_write_syslog("***** Entered into getDeliverableFilesFromDeliverables *****\n");
	ResultStatus status(0);
	int iFail = ITK_ok;
	tag_t relationType = NULLTAG;
	int secCount = 0;
	tag_t* secObjects = NULL;
	scoped_smptr<char> QCObjectStr;
	try{		
		status = AOM_ask_value_string(tQC_approANDrejObj, "object_string", &QCObjectStr);
		status = GRM_find_relation_type("IMAN_specification",&relationType);
		TC_write_syslog("relationType: IMAN_specification\n");
		TC_write_syslog("relationType Tag: %d\n",relationType);
		status = GRM_list_secondary_objects_only(tQC_approANDrejObj,relationType,&secCount,&secObjects);
		TC_write_syslog("secCount : %d\n",secCount);
		if(secCount > 0){
			status = RELSTAT_add_release_status(t_release_status_QCOneReject,secCount,secObjects,true);
			TC_write_syslog("Setting release status on Deliverable Files of target attachment: %s\n",QCObjectStr.getString());
		}
		else{
			TC_write_syslog("No Deliverable Files found for: %s\n",QCObjectStr.getString());
		}
	}
	catch (const IFail& ex)
	{
	...
	TC_write_syslog("***** Exit from getDeliverableFilesFromDeliverables *****\n");
}
//Main Function
int CheckIsApproved_andRemovetargetobj(EPM_action_message_t msg)
{
	...
	try
	{
		TC_write_syslog("******************** CheckIsApproved_andRemovetargetobj Execution Started *************************************\n");
		...
		//Reading Preference
		int ipref_count = 0;
		scoped_smptr<char*> s_pref_values;
		
		status = PREF_ask_char_values("h4_PEM_is_approved_objects", &ipref_count, &s_pref_values);
		
		TC_write_syslog("Reading preference 'h4_PEM_is_approved_objects' to get workflow target objects....\n");
		
		if(ipref_count==0)
		{
			//status = EMH_store_error("EMH_severity_error",PEM_Pref_err);
			TC_write_syslog("The 'h4_PEM_is_approved_objects'  preference is not having any value\n");
		}
		else
		{
			TC_write_syslog("Preference value count: %d\n",ipref_count);
			TC_write_syslog("Preference Values:\n");
			for(int i=0; i<ipref_count; i++)
			{
				TC_write_syslog("%s\n",s_pref_values[i]);
			}
			
			TC_write_syslog("Preference 'h4_PEM_is_approved_objects' reading successfull....\n");
		}
