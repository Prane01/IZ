...
int ITK_user_main(int argc, char* argv[])
{
	...
	if ((tc_strcmp(uname, "") != 0) && (tc_strcmp(pass, "") != 0) && (tc_strcmp(grp, "") != 0)) {  // Validates all values are filled
		if (ifail = ITK_init_module(uname, pass, grp) == ITK_ok) {
			...
			ifail = ITEM_find_rev("001029", "A", &multiple[0]);
			checkifail();
			ifail = EPM_find_process_template("Split Secondary Objects To Sub Process", &process_template);
			checkifail();
			ifail = EPM_create_process("Split Secondary Objects To Sub Process", NULL, process_template, 1, multiple, att_types, &new_process);
			checkifail();
			do {
...
void executeTask(tag_t task, const char* item_id, const char* rev_id, tag_t* userss) {
	...
	int c = 0;
	const char* desc = "@10 SET FROM ITK WRKFLW AS IT HAD NO DESC";
	char* name = NULL;
	char* TaskName = NULL;
	...
	AOM_ask_value_string(task, "object_name", &TaskName);
	cout << name << endl;
	if (tc_strcmp(TaskName, "Desc:No") == 0)
	{
		cout << TaskName << endl;
		ifail = EPM_ask_attachments(root_task, EPM_reference_attachment, &c, &attachments);
		checkifail();
		ITK_set_bypass(true);
		for (int i = 0; i < c; i++)
		{
