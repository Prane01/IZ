	...
	if ((tc_strcmp(uname, "") != 0) && (tc_strcmp(pass, "") != 0) && (tc_strcmp(grp, "") != 0)) {  // Validates all values are filled
		if (ITK_init_module(uname, pass, grp) == ITK_ok) {
			...
			for (int i = 0; i < no_grps; i++)
			{
				...
				if (tc_strcmp(value, "NewGroup") == 0) {
					cout << "Group : " << value << endl;
					for (int j = 0; j < no_users; j++)
					{
						tag_t User = NULLTAG;
						AOM_ask_value_string(members[j], "user_name", &wholeName);
						cout << "User : "<< wholeName << endl;
						SA_find_user2(wholeName, &User);
						AOM_lock(User);
						SA_set_user_status(User, status);
					...
					for (int z = 0; z < no_roles; z++)
					{
						...
						AOM_delete(roles[z]);
						cout << "Deleted role: " << value1 << endl;
						AOM_save_without_extensions(tGroup[i]);
					}
					AOM_delete(tGroup[i]);
					cout << "Deleted group: " << value << endl;
				}
			}
		}
		else
