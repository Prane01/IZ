...
void printProperties(tag_t tBomChildren)
{
	...
	int ifail = AOM_ask_value_tag(tBomChildren, "bl_revision", &trevTag);
	printf("\n trevTag :%d , ifail :%d", trevTag, ifail);
	...
	//printf("\n\t Element Effectivity : %s", tempElemEffect.c_str());
	/*
	BOM_line_look_up_attribute("bl_rev_has_variants", &iAttribute); //bl_rev_has_variants //bl_variant_state_int
	BOM_line_ask_attribute_string(tBomChildren, iAttribute, &blAttr);
	string tempVariant = replaceQuote(freeAndGetValue(blAttr));
	printf("\n\t Variant : %s", tempVariant.c_str());
	*/
	int blVariant = 0;
	...
	date_t attr_date1;
	AOM_ask_value_date(trevTag, "date_released", &attr_date1);
	string tempDateRelease = getFormattedDate(attr_date1);
	...
	date_t attr_date2;
	AOM_ask_value_date(trevTag, "last_mod_date", &attr_date2);
	string tempLastModDate = getFormattedDate(attr_date2);
	...
	date_t attr_date3;
	AOM_ask_value_date(trevTag, "creation_date", &attr_date3);
	string tempBlCreationDate = getFormattedDate(attr_date3);
	//printf("\n\t Creation Date : %s", tempBlCreationDate.c_str());
	...
	/*
	bool tempTraceLinkBool;
	string tempTraceLink = "";
	BOM_line_look_up_attribute("bl_has_trace_link", &iAttribute); //fnd0bl_has_trace_link
	BOM_line_ask_attribute_logical(tBomChildren, iAttribute, &tempTraceLinkBool);
	printf("\n\t Trace Link : %d", tempTraceLinkBool);

	if (tempTraceLinkBool == true) { tempTraceLink = "true"; }
	else if (tempTraceLinkBool == false) { tempTraceLink = "false"; }
	printf("\n\t Trace Link : %s", tempTraceLink.c_str());
	*/
	...
	/*
	tag_t blSubTag = NULLTAG;
	BOM_line_look_up_attribute("bl_substitute_list", &iAttribute);
	BOM_line_ask_attribute_tag(tBomChildren, iAttribute, &blSubTag);
	AOM_ask_value_string(blSubTag, "object_string", &blAttr);
	string tempSubstituteList = replaceQuote(freeAndGetValue(blAttr));
	printf("\n\t Substitute List : %s", tempSubstituteList.c_str());
	*/
	...
	/*
	tag_t blUomTag = NULLTAG;
	BOM_line_look_up_attribute("bl_uom", &iAttribute);
	BOM_line_ask_attribute_tag(tBomChildren, iAttribute, &blUomTag);
	AOM_ask_value_string(blUomTag, "object_string", &blAttr);
	string tempUom = replaceQuote(freeAndGetValue(blAttr));
	printf("\n\t Unit Of Measure : %s", tempUom.c_str());
	*/
	...
	/*
	tag_t blPartnerTag = NULLTAG;
	BOM_line_look_up_attribute("vm1Partners", &iAttribute);
	BOM_line_ask_attribute_tag(tBomChildren, iAttribute, &blPartnerTag);
	AOM_ask_value_string(blPartnerTag, "object_string", &blAttr);
	string tempPartner = replaceQuote(freeAndGetValue(blAttr));
	printf("\n\t Partners : %s", tempPartner.c_str());
	*/
	...
	BOM_line_look_up_attribute("bl_bomview_uid", &iAttribute);
	BOM_line_ask_attribute_string(tBomChildren, iAttribute, &blAttr);
	string tempUidBomView = replaceQuote(freeAndGetValue(blAttr));
	printf("\n\t UID of the BOMView : %s", tempUidBomView.c_str());

	BOM_line_look_up_attribute("bl_appearance_uid", &iAttribute);
	BOM_line_ask_attribute_string(tBomChildren, iAttribute, &blAttr);
	string tempUidApp = replaceQuote(freeAndGetValue(blAttr));
	printf("\n\t UID of appearance : %s", tempUidApp.c_str());

	BOM_line_look_up_attribute("bl_rev_current_id_uid", &iAttribute);
	BOM_line_ask_attribute_string(tBomChildren, iAttribute, &blAttr);
	string tempUidRevCurrent = replaceQuote(freeAndGetValue(blAttr));
	printf("\n\t Rev current uid : %s", tempUidRevCurrent.c_str());
	*/

	//need_new_csv_file();
	//printVectorToFile(ECNAttr);
	//printVectorToFile(solItem);
	...
	std::string sLine = "\"" + tempElemEffect + "\",\"" +
		std::to_string(blVariant) + "\",\"" +
		tempRuleConfigBy + "\",\"" +
		tempChildItemId + "\",\"" +
		tempItemRevID + "\",\"" +
		tempRevName + "\",\"" +
		...
		tempMakeBuy + "\",\"" +
		tempBlCreationDate + "\",\"" +
		tempSubstituteListUif + "\",\"" +
		tempUomUif + "\",\"" +
		tempPartnerUif + "\",\"" +
		std::to_string(blEndItemAsmState) + "\",\"" +
		std::to_string(blPackCount) + "\",";
...
int get_type_objects(char* searchCriteria, const char* startDate, const char* endDate) {
	...
	if (status == ITK_ok) {
		...
		for (int ii = 0; ii < n_items; ii++)
		{
			...
			//char* temp_uid = NULL;
			//char* ecnItemId = NULL;
			//char* ecnItemName = NULL;
			//char* ecnItemSBG = NULL;
			//char* ecnItemGBE = NULL;
			//char* ecnItemLOB = NULL;
			//char* ecnItemACId = NULL;
			...
			if (isSubstringContains(hpnObjectTypeStr, "H4_OrderSpecRevision") == 0 || isSubstringContains(hpnObjectTypeStr, "H4_Com_Hon_PartRevision") == 0) {
				...
				if (tc_strcmp(gov_classStr.c_str(), "ESO1.0") == 0 || tc_strcmp(gov_classStr.c_str(), NULL) == 0 || tc_strcmp(gov_classStr.c_str(), "") == 0)
				{
					...
					if (tc_strcmp(hpnPartCategoryTemp.c_str(), "Make Finished Good") == 0 || tc_strcmp(hpnPartCategoryTemp.c_str(), "Buy Finished Good") == 0) {
						...
						printf("\nBOM Line tag is : %d, BOM Line count is %d\n", tBOMLine, nBomLineCount);
						...
						if (nBomLineCount > 0)
						{
							...
							std::string sLine = "\"" + ParentItemID_temp + "\",\"\",\"" + asmbly + "\",\"\",";
			...
			//else {printf("\n No type obj");}
			//SAFE_MEM_free(hpnObjectType);

			/*SAFE_MEM_free(temp_uid);
			SAFE_MEM_free(ecnItemId);
			SAFE_MEM_free(ecnItemName);
			SAFE_MEM_free(ecnItemSBG);
			//SAFE_MEM_free(ecnItemSBU);
			SAFE_MEM_free(ecnItemGBE);
			SAFE_MEM_free(ecnItemLOB);
			SAFE_MEM_free(ecnItemACId);*/
...
int encodeCSV(const char* fileName) {
...
void create_csv_file(const char* path, const char* fileName) {
	...
	//encodeCSV(filepathlocation);

	// Create an object of ofstream to write to a file in binary mode
	file.open(filepathlocation, std::ios::binary);
	...
	unsigned char bom[] = { 0xEF, 0xBB, 0xBF };
	size_t written = fwrite(bom, sizeof(unsigned char), sizeof(bom), file);
	if (written != sizeof(bom)) {
		printf("Error writing BOM");
		//return 1;
	}
	*/
	...
	std::string sLine =
		"Parent BOM ID,Child BOM ID,Assembly(A)/Component(C),Level,"
		"Element Effectivity,Variant,Rule Configured By,ID,Revision,"
		"Revision Name,Description,Is Negative,SAP Transfer Required,"
		"SAP Transfer Status,ERP Plant,Gold Business Enterprise(GBE),"
		"Line of Business(LOB),Inventory Unit Of Measure,Purchase Unit of Measure,"
		"Managing Location,Maturity,Net Weight,Part Category,"
		"Accolade Project ID,Product Hierarchy,Design Authority,"
		"Strategic Business Unit(SBU),Strategic Business Group(SBG),Weight Unit,"
		"Usage,Packing List,All Notes,Conversion factor,Has Substitutes,"
		"Substitute,Has Global Alternates,Item Has Global Alternates,"
		"Global Alternate List,Suppressed,Hon Notes,Reference Designator,"
		"Sequence,Quantity,Release Status,Date Released,Release Effectivity,"
		"Owner,Group ID,Last Modifying User,Is Variant Item,Active Arrangement,"
		"Vendor Parts,Trace Link,HEC,TEC,Government Classification,"
		"Item Revision's Owning Project,Item Revision Type,Last Modified Date,"
		"Type of Part Object,Weight,Is ECAD component?,Comments,Legacy Part ID,"
		"MRP Alias,Material Group,Regulatory Impact,Is Library Part,Make/Buy,"
		"Creation_Date,Substitute List,Unit Of Measure,Partners,"
		"End Item Assembly State,Pack Count,TCUID";
...
void bom_sub_child(tag_t *tBomChildren, int iChildCount, std::vector<std::string> ECNAttr, std::vector<std::string> solItem, std::string parentBom, std::string asmbly, int temp_bom_level, std::string ParentItemID_temp, std::string ItemUid_temp)
{
	...
	for (int i = 0; i < iChildCount; i++)
	{
		...
		if (iNumberOfSubChild > 0)
		{
			...
			std::string sLine = "\"" + ParentItemID_temp + "\",\"" + tempChildItemId + "\",\"" + asmbly + "\",\"" + std::to_string(bom_level) + "\",";
