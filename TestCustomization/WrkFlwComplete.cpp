...
int ITK_user_main(int argc, char* argv[])
{
	...
	if ((tc_strcmp(uname, "") != 0) && (tc_strcmp(pass, "") != 0) && (tc_strcmp(grp, "") != 0)) {  // Validates all values are filled
		if (ifail = ITK_init_module(uname, pass, grp) == ITK_ok) {
			...
			tag_t task = NULLTAG;
			int mem = 0;
			/*const char* item_id = "000675";
			const char* rev_id = "A";*/
			tag_t* multiple = new tag_t[2];
			...
			checkifail();
			ifail = ITEM_find_rev("000678", "A", &multiple[1]);
			checkifail();
			ifail = EPM_find_process_template("Run throug ITK Test", &process_template);
			checkifail();
			ifail = EPM_create_process("Run throug ITK Test", NULL, process_template, 2, multiple, att_types, &new_process);
			checkifail();
			do{
				ifail = AOM_ask_value_tags(multiple[0], "fnd0MyWorkflowTasks", &n_tasks, &tasks);
				checkifail();
				if (n_tasks != 0)
				{
					task = tasks[0];
...
void executeTask(tag_t task,const char* item_id,const char* rev_id, tag_t* userss) {
	...
	int c = 0;
	const char* desc = "@06 SET FROM ITK WRKFLW";
	char* name = NULL;
