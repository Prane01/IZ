...
int ITK_user_main(int argc, char* argv[])
{
	...
	if ((tc_strcmp(uname, "") != 0) && (tc_strcmp(pass, "") != 0) && (tc_strcmp(grp, "") != 0)) {  // Validates all values are filled
		if (ifail = ITK_init_module(uname, pass, grp) == ITK_ok) {
			...
			char* date = NULL;
			date_t ddate;
			ifail = ITEM_find_rev("ECR-000004", "A", &tRevision);
			checkNullTag(tRevision);
			AOM_ask_value_date(tRevision, "date_released", &ddate);
			cout << ddate.year << endl;
			PROP_ask_composite_property(tRevision, "date_released", &date);
			cout << date << endl;
		}
		else
		{
