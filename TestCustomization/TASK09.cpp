...
int ITK_user_main(int argc, char* argv[])
{
	...
	if ((tc_strcmp(uname, "") != 0) && (tc_strcmp(pass, "") != 0) && (tc_strcmp(grp, "") != 0)) {  // Validates all values are filled
		if (ITK_init_module(uname, pass, grp) == ITK_ok) {
			cout << "\nLogin successful...\n\n";
			ITEM_find_rev("000218", "A", &tParent);
			checkNullTag(tParent);
			cout << "found parent..\n\n";

			ITEM_find_rev("000863", "A", &trevision);
			...
			for (int i = 0; i < c; i++)
			{
				...
				if (strcmp(bl_item_id, "000863") == 0)
				{
					ifail = AOM_ask_value_tag(children[i], "bl_occurrence", &childtag);
					checkNullTag(childtag);
					ifail = AOM_set_value_string(childtag, "bl_quantity", "4");
					checkifail();
					cout << "updated text.." << endl;
					AOM_save_without_extensions(childtag);
					AOM_save_without_extensions(tbvrP[0]);
					AOM_refresh(childtag, false);
