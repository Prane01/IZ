...
extern "C" {
	...
	extern DLLAPI int PLM_execute_callback1(int* decision, va_list argv) {
		*decision = ALL_CUSTOMIZATIONS;
		...
		status = METHOD_add_action(tmethod_id, METHOD_post_action_type, (METHOD_function_t)PLM_add_postAction, NULL);
		cout << "OK\n\n";
		return status;
	}
	...
	extern DLLAPI int PLM_add_preAction(METHOD_message_t* msg, va_list args) {
		...
		cout << "--> Naming already present Datasets...\n\n";
		...
		GRM_list_secondary_objects_only(primary_object, NULLTAG, &secCount, &secDatasets);
		...
		for (int i = secCount - 1; i >= 0; i--)
		{
			...
			if (count <= datasetsCount && tc_strcmp(className, "Dataset") == 0) {
				char num[100] = { 0 };
				sprintf(num, "%d", count);
				AOM_ask_value_string(primary_object, "item_id", &item_id);
				//cout << item_id << endl;
				AOM_ask_value_string(primary_object, "item_revision_id", &item_revision_id);
				//cout << item_revision_id << endl;
				AOM_ask_value_string(primary_object, "object_name", &base);
				//cout << base << endl;
	...
	extern DLLAPI int PLM_add_postAction(METHOD_message_t* msg, va_list args) {
		...
		for (int i = 0; i < secCount; i++)
		{
			POM_class_of_instance(secondary_object, &class_id);
			POM_name_of_class(class_id, &className);
			if (tc_strcmp(className, "Dataset") == 0) {
				char num[100] = { 0 };
				sprintf(num, "%d", datasetsCount+1);
				...
				AOM_set_value_string(secondary_object, "object_name", newName);
