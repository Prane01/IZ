...
extern "C" {
	...
	extern DLLAPI int set_preference_value(METHOD_message_t* msg, va_list args) {
		...
		ifail = PREF_ask_char_values("set_property_update_on_both", &prefCount, &value);
		checkifail();
		cout << value << endl;
		int j = 0;
	...
	void set_value_on_prop02(const char* item_id) {
		...
		AOM_set_value_string(tItem, "a2P01", alpha);
		AOM_set_value_string(tItem, "a2P03", p1);
