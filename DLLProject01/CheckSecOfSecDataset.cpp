...
extern "C" {
	...
	extern DLLAPI int add_custom_status(EPM_rule_message_t msg);
	...
	extern DLLAPI int PLM_execute_callback1(int* decision, va_list argv) {
		*decision = ALL_CUSTOMIZATIONS;
		...
		EPM_register_rule_handler("custom-check-sec-dataset", "Creating custom status", (EPM_rule_handler_t)add_custom_status);
		return status;
	}
